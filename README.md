...
- `AI Code Companion` has become smarter to write test cases for selcted code using multiple approaches, users can write test cases for code with following options:
  - Users can selext the code in editor and write Prompt to write test cases
  - Users can also use Context Menu options to write test cases and extension gives full freedom to developers to configure the libraries for writing the test cases.
    - Create a file at root of your project with name `.aicodecompanion.config.js` to define testing libraries for the project workspace for every programming language like below. You can also define your own custom prompt in case you dont like the prompt generated by the companion for you, see as an example for unitTests under javascript:
      ```
        module.exports = {
          testingLibraries: {
      ...
      ```
    - In case you don't provide the above config and still use the context menu then AI Code Companion is smart enough to recommend you the library names to pick from popular libraries for programing language. You will see below UI to pick the libraries and once you select it will remember the choice for future. You can reset the option by executing the command `Reset Testing Libraries Options for Unit and E2E Tests` using `cmd/ctr+shift+p`.
      <img src="./assets-readme/extension-testing-libraries-options.jpg" alt="AI Code Companion - Testing libraries options pick screenshot" width="33%" />
